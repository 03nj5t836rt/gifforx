...
REM Set path for ffmpeg and ffprobe
...
for /f "tokens=1,2" %%a in ('%FFPROBE_PATH% -v error -select_streams v:0 -show_entries stream^=height -of csv^=s^=x:p^=0 "%input_file%"') do (
...
%FFMPEG_PATH% -y -v warning -stats -i "%input_file%" -vf "fps=%fps%,scale=-1:!current_height!:flags=spline,palettegen" -update 1 -frames:v 1 "%palette_file%"
if errorlevel 1 (
...
REM Generate GIF
%FFMPEG_PATH% -y -v warning -stats -i "%input_file%" -i "%palette_file%" -filter_complex "[0:v] fps=%fps%,scale=-1:!current_height!:flags=spline [x]; [x][1:v] paletteuse" -c:v gif "%output_file%"
if errorlevel 1 (
